...
function matchCard(p){
  ...
  if(!entradas.length){ cont.appendChild(el(`<span class="sub">Sin predicciones</span>`)); }
  // ordenar por puntos desc cuando ya se jugó
  if(p.jugado) entradas.sort((a,b)=>(b[1].puntos||0)-(a[1].puntos||0));
  entradas.forEach(([nom,pr])=>{
    ...
    cont.appendChild(el(`<div class="pred ${cls}">
      <span class="who">${esc(nom)}</span>
      <span class="val">${esc(marcador(pr))}</span>${ptHtml}
    </div>`));
  });
  return c;
}
...
let dA=null, dB=null;
function vDuelos(m){
  if(!dA) dA = D.jugadores[0];
  if(!dB) dB = D.jugadores[1] || D.jugadores[0];
  m.innerHTML = "";
  const head = el(`<div class="card"><div class="selrow">
    <div class="sel"><label>Jugador A</label><select id="dA">${D.jugadores.map(j=>`<option ${j===dA?"selected":""}>${esc(j)}</option>`).join("")}</select></div>
    <div class="sel"><label>Jugador B</label><select id="dB">${D.jugadores.map(j=>`<option ${j===dB?"selected":""}>${esc(j)}</option>`).join("")}</select></div>
  </div><div id="dbody"></div></div>`);
  m.appendChild(head);
  $("#dA",head).onchange = e=>{dA=e.target.value; vDuelos(m);};
  $("#dB",head).onchange = e=>{dB=e.target.value; vDuelos(m);};
  const body = $("#dbody",head);

  const ca = D.clasificacion.find(x=>x.jugador===dA), cb = D.clasificacion.find(x=>x.jugador===dB);
  body.appendChild(el(`<div class="scoreboard">
    <div><div class="big" style="color:var(--acc2)">${ca?ca.total:0}</div><div class="nm">${esc(dA)}</div></div>
    <div class="dash">vs</div>
    <div><div class="big" style="color:#3aa0ff">${cb?cb.total:0}</div><div class="nm">${esc(dB)}</div></div>
  </div>`));

  if(dA===dB){ body.appendChild(el(`<div class="empty">Elige dos jugadores distintos 🙂</div>`)); return; }

  // partidos jugados donde difieren los puntos -> cara a cara
  let ganaA=0, ganaB=0;
  const jugados = D.partidos.filter(p=>p.jugado && p.predicciones[dA] && p.predicciones[dB]);
  body.appendChild(el(`<div class="sub" style="margin:4px 2px 10px">${jugados.length} partidos jugados en común</div>`));
  jugados.forEach(p=>{
    const pa=p.predicciones[dA].puntos||0, pb=p.predicciones[dB].puntos||0;
    if(pa>pb) ganaA++; else if(pb>pa) ganaB++;
    const teams=(p.equipos||p.codigo.split("-"));
    body.appendChild(el(`<div class="duel-line">
      <div class="side ${pa>pb?"win":""}">${esc(marcador(p.predicciones[dA]))} <b>+${pa}</b></div>
      <div class="mid">${esc(teams[0])}-${esc(teams[1])}<br>${p.resultado.local}-${p.resultado.visitante}</div>
      <div class="side ${pb>pa?"win":""}"><b>+${pb}</b> ${esc(marcador(p.predicciones[dB]))}</div>
    </div>`));
  });
  if(!jugados.length) body.appendChild(el(`<div class="empty">Todavía no hay partidos jugados.<br>¡Esto se llenará pronto! 🔥</div>`));
  else body.insertBefore(el(`<div class="scoreboard"><div><div class="big">${ganaA}</div><div class="nm">mejor ${esc(dA)}</div></div>
     <div class="dash">·</div><div><div class="big">${ganaB}</div><div class="nm">mejor ${esc(dB)}</div></div></div>`), body.children[1]);
}
